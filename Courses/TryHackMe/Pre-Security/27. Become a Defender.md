...
- Mitigation: During an attack, taking action to limit damage (such as blocking traffic, isolating affected systems, disabling compromised accounts).
- Analysis: Investigating the attack, how it happened, which systems were affected (through logs for example).
- Response/Improvement: Recovering from the attack, improving defenses to reduce risks from a similar event in the future.
...
After understanding your environment, by mapping the client's infrastructure and gain visibility, your responsibility is to defend it, building foundations and understanding how it can be protected, how it could be abused, and apply protections to reduce risk.
