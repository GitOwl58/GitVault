...
- Doesn't encrypt anything — a DNSSEC response can still be read in plaintext by anyone on-path. That's [[DNS over TLS (DoT)|DoT]]/[[DoH]]'s job, not DNSSEC's
...
- [[DNS over TLS (DoT)|DoT]]/[[DoH]] protect the **path** (query can't be read or altered in transit)
